...
DUK_LOCAL duk_idx_t duk__get_idx_func(duk_hthread *thr, duk_idx_t num_stack_args) {
	...
	idx_func = (off_stack_top - off_stack_all) / sizeof(duk_tval);
	return idx_func;
}
...
DUK_INTERNAL
duk_int_t duk_handle_call_protected(duk_hthread *thr,
                                    duk_idx_t num_stack_args,
                                    duk_small_uint_t call_flags) {
	duk_context *ctx;
	duk_size_t entry_valstack_bottom_index;
	...
	DUK_DD(DUK_DDPRINT("duk_handle_call_protected: thr=%p, num_stack_args=%ld, "
	                   "call_flags=0x%08lx (protected=%ld, ignorerec=%ld, constructor=%ld), "
...
DUK_LOCAL void duk__handle_call_error(duk_hthread *thr,
	...
	/* Side effects should not be an issue here: thr->heap and
	 * thr->heap->lj have a stable pointer.  Finalizer runs etc
	 * capture even out-of-memory errors so nothing should
	 * throw here.
	 */
...
#if defined(DUK_USE_DEBUGGER_SUPPORT)
	if (DUK_HEAP_IS_DEBUGGER_ATTACHED(thr->heap)) {
		DUK_DD(DUK_DDPRINT("returning to (possible) ecmascript activation with debugger enabled, force interrupt"));
		DUK_ASSERT(thr->interrupt_counter <= thr->interrupt_init);
		thr->interrupt_init -= thr->interrupt_counter;
		thr->interrupt_counter = 0;
		thr->heap->dbg_force_restart = 1;
...
DUK_LOCAL void duk__handle_safe_call_shared(duk_hthread *thr,
	...
	/* Side effects should not be an issue here: thr->heap and thr->heap->lj
	 * have a stable pointer.  Finalizer runs etc capture even out-of-memory
	 * errors so nothing should throw here.
	 */
	DUK_TVAL_SET_UNDEFINED_UPDREF(thr, &thr->heap->lj.value1);  /* side effects */
	DUK_TVAL_SET_UNDEFINED_UPDREF(thr, &thr->heap->lj.value2);  /* side effects */
